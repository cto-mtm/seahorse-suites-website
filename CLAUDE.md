...
- **Floor/access:** Units 1, 2, 3, 4 are on the second floor, accessible only by stairs. Unit 5 (Beachside Escape) is on the ground floor. Units 1, 2, 3 share a balcony. Unit 4 (Ocean Breeze) has its own private wrap-around balcony.
- Third-party imagery (e.g. /explore attractions) lives in `photos/explore/<slug>/` with a `CREDITS.md` recording source + license — only public-domain or properly licensed files (never scraped from Google/business sites). Attractions without licensed photos render gradient fallbacks.
